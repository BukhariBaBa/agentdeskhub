          ...
          React.createElement(
            "span",
            {
              style: {
                ...
                background: i <= cur ? "var(--indigo-600)" : "#fff",
                color: i <= cur ? "#fff" : "var(--text-subtle)",
                border:
                  "1.5px solid " +
                  (i <= cur ? "var(--indigo-600)" : "var(--border-strong)"),
              },
            },
            i < cur
          ...
          React.createElement(
            "span",
            {
              style: {
                fontSize: 11.5,
                fontWeight: 550,
                color: i <= cur ? "var(--text)" : "var(--text-subtle)",
              },
            },
            s,
          ),
...
function ProposalsView({ app }) {
  ...
  const detail =
    app.detail && app.proposals.find((p) => p.id === app.detail.id);

  const generate = () => {
    setRunning(true);
    setTimeout(() => {
      setRunning(false);
      app.runAgent("proposal");
    }, 900);
  };
  ...
  return React.createElement(
    ...
    React.createElement(PageHeader, {
      ...
      actions: React.createElement(
        ...
        {
          variant: "primary",
          icon: running ? undefined : "penTool",
          onClick: generate,
          disabled: running,
        },
        running
    ...
    app.proposals.length === 0
      ? React.createElement(EmptyState, {
          ...
          body: "Let the Proposal Agent turn a client brief into a polished, ready-to-send proposal.",
          cta: "Generate a proposal",
          onCta: generate,
        })
      : React.createElement(
          DataTable,
          ...
          app.proposals.map((p) =>
            React.createElement(
              Row,
              { key: p.id, onClick: () => app.go("proposals", { id: p.id }) },
              React.createElement(
                Td,
                null,
                React.createElement(
      ...
      React.createElement(
        ...
        p.status === "Draft"
          ? React.createElement(
              Button,
              {
                ...
                onClick: () =>
                  app.toast(
                    "Proposal sent — now visible in the client portal",
        ...
        React.createElement(
          Button,
          {
            ...
            icon: "download",
            onClick: () =>
              app.toast("Downloading proposal PDF…", { icon: "download" }),
          },
          "PDF",
        ),
      ),
  ...
  return React.createElement(
    "div",
    {
      style: {
        ...
        boxShadow:
          acc || rec
            ? "0 0 0 1px " + (acc ? "var(--emerald-500)" : "var(--indigo-600)")
            : "none",
      },
    },
    (rec || acc) &&
      React.createElement(
        ...
        {
          style: {
            background: acc ? "var(--emerald-500)" : "var(--indigo-600)",
            color: "#fff",
            fontSize: 10.5,
            fontWeight: 600,
            textTransform: "uppercase",
