    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          "Protected by admin auth · all actions are logged",
        ),
      ),
      React.createElement(
        "p",
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        "Every admin write action is recorded with actor + timestamp.",
      ),
      React.createElement(
        "div",
...
function AdminApp() {
  const [authed, setAuthed] = useStateApp(() => {
    try {
      return sessionStorage.getItem("agentdesk_admin") === "1";
    } catch (e) {
      return false;
    }
  });
  ...
  const [layout, setLayout] = useStateApp(
    window.innerWidth < 1100 ? "narrow" : "wide",
  );
  ...
  const toast = (message, icon, tint) => {
    ...
    setToasts((t) => [
      ...t,
      { id, message, icon: icon || "checkCircle", tint: tint || "indigo" },
    ]);
  ...
  const act = (action, target) => {
    setAudit((a) => [
      {
        id: "au" + Date.now(),
        ts: "2026-06-09 " + new Date().toTimeString().slice(0, 5),
  ...
  const login = () => {
    try {
      sessionStorage.setItem("agentdesk_admin", "1");
    } catch (e) {}
    setAuthed(true);
  };
  const logout = () => {
    try {
      sessionStorage.removeItem("agentdesk_admin");
    } catch (e) {}
    setAuthed(false);
  };
  ...
  if (!authed)
    return React.createElement(
      React.Fragment,
      null,
      React.createElement(AdminLogin, { onLogin: login }),
  ...
  return React.createElement(
      ...
      React.createElement(
        "aside",
        {
          style: {
            ...
            display: "flex",
            flexDirection: "column",
            position: layout === "wide" ? "sticky" : "fixed",
            top: 0,
            height: "100vh",
            zIndex: 90,
            flexShrink: 0,
          ...
          NAV.map((n) => {
            ...
            return React.createElement(
              "button",
              {
                ...
                onMouseEnter: (e) => {
                  if (!on) e.currentTarget.style.color = "var(--side-text-hi)";
                },
                onMouseLeave: (e) => {
                  if (!on) e.currentTarget.style.color = "var(--side-text)";
                },
              },
              React.createElement(Icon, {
                name: n.icon,
    ...
    React.createElement(
      ...
      React.createElement(
        "div",
        {
          style: {
            flex: 1,
            padding: layout === "narrow" ? "20px 16px 60px" : "28px 32px 60px",
            maxWidth: 1240,
            width: "100%",
            margin: "0 auto",
            boxSizing: "border-box",
    ...
    React.createElement(AuditDrawer, {
      open: auditOpen,
