...
const SIGNUPS_30D = [
  6, 9, 7, 12, 8, 5, 4, 11, 14, 10, 7, 9, 13, 16, 12, 8, 6, 10, 15, 18, 14, 11,
  9, 13, 17, 21, 16, 12, 19, 24,
];
...
const ADMIN_WORKSPACES = WS_NAMES.map((w, i) => ({
  ...
  nextBilling:
    "2026-0" +
    ((i % 6) + 4).toString().padStart(1, "0") +
    "-" +
    (((i * 7) % 27) + 1).toString().padStart(2, "0"),
}));

/* ---------- Subscriptions (from LemonSqueezy webhooks) ---------- */
const ADMIN_SUBSCRIPTIONS = ADMIN_WORKSPACES.filter(
  (w) => w.status !== "Trial",
).map((w) => ({
  id: "sub_" + w.id,
  workspace: w.name,
...
const ADMIN_RUNS = Array.from({ length: 40 }, (_, i) => {
  ...
  return {
    ...
    tokens,
    model,
    duration: (1.2 + (i % 7) * 0.6).toFixed(1) + "s",
    input:
      agent === "Lead Scout"
...
const ADMIN_ANNOUNCEMENTS = [
  {
    ...
    date: "2026-06-02",
    target: "All users",
    title: "Follow-up Agent is live",
    message:
      "Your sixth agent is here — Follow-up Agent automatically nudges clients on unpaid invoices and stale proposals. Enable it from the Agents page.",
    reach: 1284,
  },
  {
    id: "a2",
    date: "2026-05-21",
    target: "Pro only",
    title: "Custom branding for client portals",
    message:
      "Pro workspaces can now add a logo and accent color to every client portal. Head to Settings → Portal.",
    reach: 689,
  },
  {
    id: "a3",
    date: "2026-05-09",
    target: "Agency only",
    title: "Team seats + shared agent library",
    message:
      "Agency plans now include up to 5 seats and a shared library of saved agent configurations across your team.",
    reach: 142,
  },
  {
    id: "a4",
    date: "2026-04-28",
    target: "Starter only",
    title: "Upgrade and save 40%",
    message:
      "Hitting your 50-run limit? Upgrade to Pro this week with code LAUNCH40 and unlock all 6 agents at 40% off.",
    reach: 453,
  },
];
...
const ADMIN_AUDIT = [
  ...
  },
  {
    ...
    target: 'All users · "Follow-up Agent is live"',
    tint: "indigo",
  },
];
