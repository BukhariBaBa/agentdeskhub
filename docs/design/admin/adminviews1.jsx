  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        "Platform health across all AgentDesk workspaces · June 2026",
      ),
    ),
    ...
    React.createElement(
      ...
      React.createElement(MetricCard, {
        ...
        value: m.totalWorkspaces.toLocaleString(),
        delta: m.totalWorkspacesDelta,
        tint: "indigo",
      }),
      React.createElement(MetricCard, {
        icon: "users",
    ...
    React.createElement(
      ...
      React.createElement(MetricCard, {
        ...
        value: m.runsThisMonth.toLocaleString(),
        delta: m.runsDelta,
        tint: "indigo",
      }),
      React.createElement(MetricCard, {
      ...
      React.createElement(MetricCard, {
        ...
        value: m.churnedThisMonth,
        delta: m.churnDelta,
        tint: "rose",
        invertDelta: true,
        sub: "Cancelled or suspended",
      }),
    ),
    ...
    React.createElement(
      "div",
      {
        style: {
          display: "grid",
          gridTemplateColumns: admin.layout === "narrow" ? "1fr" : "1fr 1.3fr",
          gap: 14,
        },
      },
      React.createElement(
      ...
      React.createElement(
        ...
        {
          title: "Revenue trend",
          sub: "Last 12 months · MRR",
          action: React.createElement(
            Badge,
            { tint: "emerald", size: "sm", dot: true },
            fmtK(REVENUE_12M[REVENUE_12M.length - 1].v),
          ),
        },
...
function WorkspacesScreen({ admin }) {
  ...
  const ws = ADMIN_WORKSPACES.filter(
    (w) =>
      (q === "" ||
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        ADMIN_WORKSPACES.length +
          " workspaces · " +
          ADMIN_WORKSPACES.filter((w) => w.status === "Active").length +
          " active",
      ),
    ),
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(FilterTabs, {
          value: status,
          onChange: setStatus,
          options: ["All", "Active", "Trial", "Cancelled", "Suspended"],
        }),
      ),
    ),
    ...
    React.createElement(
      ATable,
      {
        ...
        columns: [
          ...
          { label: "Owner email" },
          { label: "Plan", w: 90 },
          { label: "Runs", right: true, w: 80 },
          { label: "Joined", w: 120 },
          { label: "Status", w: 120 },
          { label: "Actions", right: true, w: 90 },
        ],
      },
      ws.map((w) =>
        React.createElement(
          ...
          React.createElement(
            ACell,
            { color: "var(--text-muted)", mono: true },
            w.email,
          ),
          React.createElement(
            ACell,
          ...
          React.createElement(
            ACell,
            { color: "var(--text-muted)", mono: true, nowrap: true },
            w.joined,
          ),
          React.createElement(
            ACell,
            null,
            React.createElement(AStatus, { value: w.status }),
          ),
          React.createElement(
            ACell,
            { right: true },
    ...
    React.createElement(
      Drawer,
      { open: !!current, onClose: () => setActive(null), width: 560 },
      current &&
        React.createElement(WorkspaceDetail, {
          ws: current,
          admin,
...
function WorkspaceDetail({ ws, admin, onClose }) {
  ...
  const pays = ADMIN_PAYMENTS.filter((p) => p.workspace === ws.name).slice(
    0,
    5,
  );
  const planHistory = [
    {
      d: ws.joined,
      e: "Signed up · " + (ws.status === "Trial" ? "Trial started" : "Starter"),
    },
    ws.plan !== "Starter"
      ? { d: "2025-12-01", e: "Upgraded to " + ws.plan }
      : null,
    ws.status === "Suspended"
      ? { d: "2026-06-09", e: "Suspended by admin" }
      : null,
    ws.status === "Cancelled"
      ? { d: "2026-05-15", e: "Subscription cancelled" }
      : null,
  ].filter(Boolean);
  ...
  return React.createElement(
    ...
    React.createElement(DrawerHead, {
      ...
      sub: ws.id + " · " + ws.email,
      onClose,
      badge: React.createElement(AStatus, { value: ws.status }),
    }),
    React.createElement(
      "div",
      ...
      React.createElement(
        ...
        React.createElement(
          Button,
          {
            ...
            variant: "secondary",
            icon: "sliders",
            onClick: () => admin.toast("Opening plan editor…", "sliders"),
          },
          "Edit plan",
        ),
        ws.status === "Suspended"
          ? React.createElement(
              Button,
              {
                ...
                variant: "secondary",
                icon: "refresh",
                onClick: () => admin.act("Reinstated workspace", ws.name),
              },
              "Reinstate",
            )
          : React.createElement(
              Button,
              {
                ...
                variant: "danger",
                icon: "ban",
                onClick: () => admin.act("Suspended workspace", ws.name),
              },
              "Suspend",
            ),
        React.createElement(
          Button,
          {
            ...
            variant: "ghost",
            icon: "fileText",
            onClick: () => admin.toast("Notes saved", "fileText"),
          },
          "Notes",
        ),
      ),
        ...
        runs.length
          ? runs.map((r, i) =>
              React.createElement(
                ...
                React.createElement(
                  "span",
                  { style: { fontSize: 12.5, flex: 1, fontWeight: 500 } },
                  r.agent,
                ),
                React.createElement(
                  "span",
              ...
              React.createElement(
                "div",
                {
                  ...
                  style: {
                    ...
                    padding: "9px 12px",
                    borderBottom:
                      i < pays.length - 1 ? "1px solid var(--border)" : "none",
                  },
                },
                React.createElement(
                  "span",
