...
function Button({
  ...
  const variants = {
    primary: {
      bg: hover ? "var(--primary-hover)" : "var(--primary)",
      color: "#fff",
      border: "1px solid transparent",
    },
    secondary: {
      bg: hover ? "var(--zinc-50)" : "#fff",
      color: "var(--zinc-800)",
      border: "1px solid var(--border-strong)",
    },
    ghost: {
      bg: hover ? "var(--zinc-100)" : "transparent",
      color: "var(--zinc-600)",
      border: "1px solid transparent",
    },
    danger: {
      bg: hover ? "var(--rose-50)" : "transparent",
      color: "var(--rose-600)",
      border: "1px solid " + (hover ? "var(--rose-100)" : "transparent"),
    },
    success: {
      bg: hover ? "var(--emerald-600)" : "var(--emerald-500)",
      color: "#fff",
      border: "1px solid transparent",
    },
    dangerSolid: {
      bg: hover ? "var(--rose-600)" : "var(--rose-500)",
      color: "#fff",
      border: "1px solid transparent",
    },
  }[variant];
  ...
  return React.createElement(
    ...
    dot &&
      React.createElement("span", {
        style: { width: 5, height: 5, borderRadius: 99, background: t.solid },
      }),
    children || status,
  );
}
  ...
  return React.createElement(
    "button",
    {
      ...
      style: {
        ...
        borderRadius: "var(--r-md)",
        border: "1px solid transparent",
        background: h ? "var(--zinc-100)" : "transparent",
        color: "var(--zinc-600)",
        cursor: "pointer",
        display: "inline-flex",
        alignItems: "center",
    ...
    React.createElement(
      "div",
      {
        ...
        style: {
          ...
          animation: "ad-slide-in .22s cubic-bezier(.2,.7,.2,1)",
          display: "flex",
...
function Markdown({ text }) {
  ...
  lines.forEach((ln) => {
    if (/^#\s/.test(ln)) {
      flush();
      blocks.push({ t: "h1", c: ln.slice(2) });
    } else if (/^##\s/.test(ln)) {
      flush();
      blocks.push({ t: "h2", c: ln.slice(3) });
    } else if (/^\d+\.\s/.test(ln)) {
  ...
  const inline = (s) => {
    ...
    return parts.map((p, i) =>
      /^\*\*[^*]+\*\*$/.test(p)
        ? React.createElement(
            "strong",
            { key: i, style: { fontWeight: 650, color: "var(--text)" } },
            p.slice(2, -2),
          )
        : p,
    );
  ...
  return React.createElement(
    "div",
    { style: { color: "var(--zinc-700)", fontSize: 14, lineHeight: 1.65 } },
    blocks.map((b, i) => {
      if (b.t === "h1")
        return React.createElement(
          "h2",
      ...
      if (b.t === "ul")
        return React.createElement(
          "ul",
          { key: i, style: { margin: "4px 0", paddingLeft: 20 } },
          b.items.map((it, j) =>
            React.createElement(
              "li",
              { key: j, style: { margin: "4px 0" } },
              inline(it),
            ),
          ),
        );
      ...
      React.createElement(
        "div",
        {
          ...
          style: {
            ...
            fontSize: 13.5,
            fontWeight: 500,
            animation: "ad-toast-in .25s cubic-bezier(.2,.7,.2,1)",
            maxWidth: 420,
          },
        },
        React.createElement(
          "span",
          {
            style: {
              display: "flex",
              color: t.tint ? TINT[t.tint].solid : "var(--emerald-500)",
            },
          },
          React.createElement(Icon, {
            name: t.icon || "checkCircle",
            size: 17,
            strokeWidth: 2.2,
          }),
        ),
